...
)
{
	...
	if (context->name)
	{
		if (_app_search_isstringfound (&context->name->sr, &search_string->sr, context, &is_changed))
			goto CleanupExit;
	}
...
BOOLEAN _app_search_applyfilter (
	_In_ HWND hwnd,
	_In_ INT listview_id,
	_In_opt_ PR_STRING search_string
)
{
	...
	{
		context = (PITEM_CONTEXT)_r_listview_getitemlparam (hwnd, listview_id, i);
